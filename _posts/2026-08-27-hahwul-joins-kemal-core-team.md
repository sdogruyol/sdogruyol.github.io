...
*Originally published on the [Kemal blog](https://kemalcr.com/blog/2026/08/27/hahwul-joins-kemal-core-team/).*
...
<p style="text-align:center;margin:8px 0 28px;">
  <img src="/images/hahwul.jpg" alt="HAHWUL" style="width:132px;height:132px;border-radius:50%;object-fit:cover;margin:0 auto;box-shadow:0 8px 24px rgba(44,62,80,0.18);" />
</p>
...
That first finding — **WebSocket request smuggling** — is the kind of bug that hides in the gap between a framework and the reverse proxy in front of it. A rejected upgrade that left the connection keep-alive could let a pipelined request slip past the proxy's access controls. The fix in [Kemal 1.13.0](https://kemalcr.com/blog/2026/08/24/kemal-1.13.0-released/) closes the connection on a failed handshake, requires `GET` for upgrades per RFC 6455, and makes same-origin the default for WebSocket `Origin` checks.
...
If you run Kemal in production, [upgrade to 1.13.0](https://kemalcr.com/blog/2026/08/24/kemal-1.13.0-released/). That release is the first public snapshot of this collaboration. It will not be the last.
...
If you find something in Kemal, we still want to hear it — [open an issue](https://github.com/kemalcr/kemal/issues) or reach the team through the usual [community channels](https://kemalcr.com/community). The difference is that those reports now have a Security Lead on the other side.
